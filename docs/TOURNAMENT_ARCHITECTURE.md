...
| Coding loop | Native safety lane; Aider provisional general-edit efficiency lane | Conditional Qwen Code/Cline/Kilo experiments; screened adapters retained for Prime, Cordis, mini-SWE-agent, OpenHands, oh-my-pi and SWE-agent | Run identical edit/debug/repository tasks with the same model and tool plane. Do not install permanent services for screened contenders. |
