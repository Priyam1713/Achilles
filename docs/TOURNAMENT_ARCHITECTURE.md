...
| Coding loop | Native safety lane; Aider provisional general-edit efficiency lane | SWE-agent; screened adapters retained for Prime, Cordis, mini-SWE-agent, OpenHands, oh-my-pi | Run identical edit/debug/repository tasks with the same model and tool plane. Do not install permanent services for screened contenders. |
